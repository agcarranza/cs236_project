...
def sample_gumbel_softmax(logits, tau, hard=True):
    return torch.autograd.Variable(F.gumbel_softmax(logits, tau=tau, hard=hard)[:,0],requires_grad=True)
...
def log_normal(x, m, v):
    ...
    """
    log_prob = -(x-m).pow(2)/(2*v) - v.log()/2 - (2*np.pi*torch.ones_like(x)).log()/2
...
def evaluate_lower_bound(model, labeled_test_subset):
    check_model = isinstance(model, GVAE)
    assert check_model, "This function is only intended for GVAE"
    ...
    def compute_metrics(fn, repeat):
        ...
        for _ in range(repeat):
            nelbo, kl, rec = detach_torch_tuple(fn(xl))
    ...
    nelbo, kl, rec = compute_metrics(model.nelbo, 100)
...
def log_summaries(writer, summaries, global_step):
    for tag in summaries:
        ...
        if isinstance(val,(np.ndarray)):
            z_num = int((1+np.sqrt(1+8*val.shape[0]))/2)
            ind = (torch.tril(torch.ones((z_num,z_num)),-1)==1).nonzero()+1
            for n, v in enumerate(val):
                i, j = ind[n].detach().numpy()
                n_ind = str(i)+'-'+str(j) 
                tf_summary = tf.Summary.Value(tag=tag+'/'+n_ind, simple_value=v)
